...
# Read or fetch activation bytes (8 hex)
$authCode = ''
if (Test-Path 'audible-activation-code.txt') { $authCode = (Get-Content -Raw 'audible-activation-code.txt') }

if ([string]::IsNullOrWhiteSpace($authCode)) {
  # Try to fetch via audible-cli wrapper
  $prev = $env:AUDIBLE_AUTH_PASSWORD
  if ($AudiblePassword) {
    $plainPw = (New-Object System.Net.NetworkCredential('', $AudiblePassword)).Password
    if (-not [string]::IsNullOrWhiteSpace($plainPw)) { $env:AUDIBLE_AUTH_PASSWORD = $plainPw }
  }
  try {
    $bytes = & .\audible-cli-wrapper.ps1 activation-bytes 2>$null
    if ($bytes -and $bytes -match '^[0-9a-fA-F]{8}$') {
      $authCode = $bytes
      $authCode | Out-File -FilePath 'audible-activation-code.txt' -Encoding UTF8
      Write-Host 'Saved activation bytes to audible-activation-code.txt' -ForegroundColor Green
    }
  } catch { }
  $env:AUDIBLE_AUTH_PASSWORD = $prev
}
